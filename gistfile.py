# -*- coding: utf-8 -*-
"""
Created on Wed Jan 21 21:15:13 2015
...
# added something here, and there
# if  this does not scream conflict i dont knwo what does
...
def _sw(df, up_rows=10, down_rows=5, left_cols=4, right_cols=3, return_df=False):
    ...
    if len(df.index) > 1000:
        print "Index :", ", ".join(df.index[:500].tolist())
        print "..."
        print ", ".join(df.index[-500:].tolist())
    else:
        print "Index :", ", ".join(df.index.tolist())

    if len(df.columns) > 1000:
        print "Column:", ", ".join(list(df.columns[:500].tolist()))
        print "..." 
        print ", ".join(list(df.columns[-500:].tolist()))
    else:
